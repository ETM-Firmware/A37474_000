    ...
    switch (sdo_index) {
        ...
        case SDO_IDX_EF_REF:
            if (is_upload) {
                ...
                global_data_A37474.page_data[2] = set_value;
                ETMEEPromWritePage(0, global_data_A37474.page_data);
            }
        ...
        case SDO_IDX_EG_REF:
            if (is_upload) {
                txData[4] = ETMAnalogOutputGetSetPoint(&global_data_A37474.analog_output_top_voltage) & 0x00ff;
                txData[5] = (ETMAnalogOutputGetSetPoint(&global_data_A37474.analog_output_top_voltage) >> 8) & 0x00ff;
            } else {
                // check max, min range
                set_value = (unsigned int) data[5] << 8;
                set_value += (unsigned int) data[4];
                global_data_A37474.ethernet_top_ref = set_value;
                global_data_A37474.page_data[1] = set_value;
                ETMEEPromWritePage(0, global_data_A37474.page_data);
            }
        ...
        case SDO_IDX_EK_REF:
            if (is_upload) {
                txData[4] = ETMAnalogOutputGetSetPoint(&global_data_A37474.analog_output_high_voltage) & 0x00ff;
                txData[5] = (ETMAnalogOutputGetSetPoint(&global_data_A37474.analog_output_high_voltage) >> 8) & 0x00ff;
            } else {
                // check max, min range
                set_value = (unsigned int) data[5] << 8;
                set_value += (unsigned int) data[4];
                global_data_A37474.ethernet_hv_ref = set_value;
                global_data_A37474.page_data[0] = set_value;
                ETMEEPromWritePage(0, global_data_A37474.page_data);
            }
        ...
        case SDO_IDX_EF_READ:
            if (is_upload) {
                txData[4] = ETMAnalogInputGetReading(&global_data_A37474.input_htr_v_mon) & 0x00ff;
                txData[5] = (ETMAnalogInputGetReading(&global_data_A37474.input_htr_v_mon) >> 8) & 0x00ff;
            }
            break;

        case SDO_IDX_IF_READ:
            if (is_upload) {
                txData[4] = ETMAnalogInputGetReading(&global_data_A37474.input_htr_i_mon) & 0x00ff;
                txData[5] = (ETMAnalogInputGetReading(&global_data_A37474.input_htr_i_mon) >> 8) & 0x00ff;
            }
            break;
        ...
        case SDO_IDX_EC_READ:
            if (is_upload) {
                set_value = ETMAnalogInputGetReading(&global_data_A37474.input_bias_v_mon) / 10;
        ...
        case SDO_IDX_EG_READ:
            if (is_upload) {
                txData[4] = ETMAnalogInputGetReading(&global_data_A37474.input_top_v_mon) & 0x00ff;
                txData[5] = (ETMAnalogInputGetReading(&global_data_A37474.input_top_v_mon) >> 8) & 0x00ff;
            }
            break;

        case SDO_IDX_EK_READ:
            if (is_upload) {
                txData[4] = ETMAnalogInputGetReading(&global_data_A37474.input_hv_v_mon) & 0x00ff;
                txData[5] = (ETMAnalogInputGetReading(&global_data_A37474.input_hv_v_mon) >> 8) & 0x00ff;
            }
            break;
        ...
        case SDO_IDX_IKP_READ:
            if (is_upload) {
                set_value = ETMAnalogInputGetReading(&global_data_A37474.input_gun_i_peak) / 10;
                txData[4] = set_value & 0x00ff;
                txData[5] = (set_value >> 8) & 0x00ff;
            }
            break;
        ...
        case SDO_IDX_GD_FAULT:
            if (is_upload) {
                ...
                txData[4] = set_value & 0x00ff;
                txData[5] = (set_value >> 8) & 0x00ff;
                set_value = global_data_A37474._WARNING_REGISTER;
