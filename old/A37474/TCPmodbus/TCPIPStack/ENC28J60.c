...
#define SR_CMD  ((0x7<<5) | 0x1F)   // System Reset command does not use an address.
...
typedef struct  __attribute__((aligned(2), packed))
{
    ...
    #define SPI_ON_BIT          (ENC_SPICONbits.SSPEN)
    ...
    #define SPI_ON_BIT          (ENC_SPICONbits.ON)
#else
...
void MACInit(void)
{
    ...
    ClearSPIDoneFlag();
#if defined(__18CXX)
    ENC_SPICON = 0x20;     // SSPEN bit is set, SPI in master mode, FOSC/4,
    ...
    ENC_SPISTAT = 0;        // clear SPI
        ...
        ENC_SPICON = 0x0F;     // 1:1 primary prescale, 5:1 secondary prescale (8MHz  @ 40MIPS)
    //    ENC_SPICON = 0x1E;   // 4:1 primary prescale, 1:1 secondary prescale (10MHz @ 40MIPS, Doesn't work.  CLKRDY is incorrectly reported as being clear.  Problem caused by dsPIC33/PIC24H ES silicon bug.)
    #elif defined(__PIC24F__) || defined(__PIC24FK__)
        ENC_SPICON = 0x1B;     // 1:1 primary prescale, 2:1 secondary prescale (8MHz  @ 16MIPS)
    #else   // dsPIC30F
	//ENC_SPICON = 0x1F;     // sec 2:1(10M), should be 0x1f or 1:1 sec, 1:1 primary prescale, 1:1 secondary prescale (20MHz)
	// DPARKER EDIT HERE
	ENC_SPICON = 0b11111;	 // sec 1:1(10M), primary 1:1.
    #endif
//    ENC_SPICON2 = 0;
    ENC_SPICONbits.CKE = 1;
    ENC_SPICONbits.MSTEN = 1;
    ENC_SPISTATbits.SPIEN = 1;
#elif defined(__C32__)
    ENC_SPIBRG = (GetPeripheralClock()-1ul)/2ul/ENC_MAX_SPI_FREQ;
	ENC_SPICONbits.SMP = 1;	// Delay SDI input sampling (PIC perspective) by 1/2 SPI clock
    ENC_SPICONbits.CKE = 1;
    ENC_SPICONbits.MSTEN = 1;
    ENC_SPICONbits.ON = 1;
#endif
    ...
    WriteReg(EWRPTH, HIGH(TXSTART));
          ...
          i = ReadETHReg(EWRPTL).Val;
          i = ReadETHReg(EWRPTH).Val;
